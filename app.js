...
var playersection=document.getElementById('player-wrapper')
var playlistWrapper=document.getElementById('playlist-wrapper')

playersection.innerHTML=`<iframe id="video-player" src="https://player.vimeo.com/video/${videoPlaySectionData[0].vimeoId}" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>
...
for(var i=0;i<playlistData.length;i++){
    ...
    if(i==0){
        playlistWrapper.innerHTML+=` <div id="card${playlistData[i].id}" onclick="addActiceClass(${playlistData[i].id})" class="playlist-card active-card">
    <img class="thumbnail" src="${playlistData[i].thumbnail}" />
    <h3 class="video-card-title">${playlistData[i].title}</h3>
  </div>`
    }else{

        playlistWrapper.innerHTML+=` <div onclick="addActiceClass(${playlistData[i].id})" id="card${playlistData[i].id}" class="playlist-card">
    <img class="thumbnail" src="${playlistData[i].thumbnail}" />
    <h3 class="video-card-title">${playlistData[i].title}</h3>
  </div>`
...
function addActiceClass(id){
    $('.active-card').removeClass('active-card')
   $('#card'+id).addClass('active-card')

   playersection.innerHTML=`<iframe id="video-player" src="https://player.vimeo.com/video/${videoPlaySectionData[id-1].vimeoId}" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>
